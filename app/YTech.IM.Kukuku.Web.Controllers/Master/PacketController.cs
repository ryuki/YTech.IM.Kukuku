...
namespace YTech.IM.Kukuku.Web.Controllers.Master
{
    [HandleError]
    public class PacketController : Controller
    {
        ...
        [Transaction]
        public virtual ActionResult ListSearch(string sidx, string sord, int page, int rows, string packetId, string packetName)
        {
            int totalRecords = 0;
            var packets = _mPacketRepository.GetPagedPacketList(sidx, sord, page, rows, ref totalRecords,packetId, packetName);
            int pageSize = rows;
            int totalPages = (int)Math.Ceiling((float)totalRecords / (float)pageSize);
