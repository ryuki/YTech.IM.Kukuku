    ...
    public partial class InventoryController
    {
      ...
      ;
                ...
                rows = (
                    ...
                    select new
                    {
                      ...
                      GetCustomerName(troom.TransId.TransBy),
                         troom.RoomId != null ? troom.RoomId.RoomName : null,
        ...
        public ActionResult PrintFactur(string TransId)
        {
            ...
            {
                TTrans trans = _tTransRepository.Get(TransId);
                if (trans.TransStatus == EnumTransactionStatus.Service.ToString())
                {
                    SetReportDataForPrint(TransId);
                    report = "RptPrintFacturService";
                }
                else if (trans.TransStatus == EnumTransactionStatus.Sales.ToString())
                {
                    PrintSales(TransId);
                    report = "RptPrintFacturSales2";
                }
                Success = true;
            }
            catch (Exception ex)
