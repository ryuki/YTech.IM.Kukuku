...
<div>
    <span id="toolbar" class="ui-widget-header ui-corner-all"><a id="newTrans" href="<%= Url.Action(ViewData.Model.Trans.TransStatus.Equals(EnumTransactionStatus.PurchaseOrder.ToString()) ? "Index" : Model.Trans.TransStatus.ToString(), "Inventory") %>">
        Baru</a>
        <button id="btnPaid" name="btnPaid" type="submit">
            Simpan</button>
        <button id="btnOut" name="btnOut" type="button">
            Simpan</button>
    ...
    <tr>
        ...
        <td>
            <table>
                ...
                <% } %>
                <% if (ViewData.Model.ViewEmployee)
                   {	%>
                <tr>
                    <td>
                        <label for="EmployeeId">Salesman :</label>
                    </td>
                    <td>
                        <%= Html.DropDownList("EmployeeId", Model.EmployeeList)%>
...
function onSavedSuccess(e) {
 ...
 if (e)
 {
  ...
  if (success == false) {
                ...
                if (msg == "redirect") {
    var urlreport = '<%= ResolveUrl("~/ReportViewer.aspx?rpt=RptPrintFacturSales2") %>';
//    alert(urlreport);
...
var imgerror = '<%= Url.Content("~/Content/Images/cross.gif") %>';
    ...
    messages: {
       <% if (ViewData.Model.ViewDate) {	%> "Trans.TransDate": "<img id='TransDateerror' src='"+imgerror+"' hovertext='Tanggal tidak boleh kosong' />"  ,<% } %>
        ...
        <% if (ViewData.Model.ViewEmployee) {	%>  "EmployeeId": "<img id='EmployeeIderror' src='"+imgerror+"' hovertext='Pilih Salesman' />",<% } %>
...
//                $('#list').setColProp('ItemId', { editoptions: { value: items} });
            ...
            //alert(total);
                 $("#paymentSubTotal").text(total);
                 $("#hidPaymentSubTotal").val(total);
                 $("#paymentDiscount").text(0);
                 $("#paymentGrandTotal").text(total);
